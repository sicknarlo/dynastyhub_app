...
import { value, roundNumber } from '../../utils';
import TradeCalculator from '../../components/TradeCalculator';

export default class TradeCalculatorContainer extends Component {
  state = {
    players: [],
    team1: [],
    team2: [],
  ...
  evaluateTrade = async () => {
    ...
    const { fullTeam1, fullTeam2 } = await response.json();
    let alertCount = 0;
    this.setState({
      showResults: true,
      currentTab: '1',
  ...
  render() {
    ...
    const team1Options = players.reduce((acc, x) => {
      let value = x._id;
    ...
    const team2Options = players.reduce((acc, x) => {
      let value = x._id;
      const name = x.position === 'PICK' ? x.name : `${x.name}, ${x.position} ${x.team}`;
      acc.push({ value, label: name });
      let n = 2;
